...
func ValidatePersonalityDNA(raw json.RawMessage) (models.PersonalityDNA, error) {
	...
	if len(dna.Planets) < 3 || len(dna.Planets) > 7 {
		return dna, fmt.Errorf("planets must contain 3-7 items")
	}
	for i, planet := range dna.Planets {
		if length(planet.Name) < 2 || length(planet.Name) > 40 {
			return dna, fmt.Errorf("planets.%d.name must be 2-40 characters", i)
		}
		if length(planet.Meaning) > 180 {
			return dna, fmt.Errorf("planets.%d.meaning must be 180 characters or fewer", i)
		}
		if planet.Energy < 0 || planet.Energy > 100 {
			return dna, fmt.Errorf("planets.%d.energy must be 0-100", i)
		}
	}
