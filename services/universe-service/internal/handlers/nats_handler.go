...
const (
	...
	failedEventMessageStage  = ":universe-failed"
...
var ErrInvalidCompositionCommand = errors.New("invalid universe composition command")
...
// NATSHandler owns the Universe service's transport-specific request handling.
...
func (handler *NATSHandler) PublishCompositionFailure(ctx context.Context, message *nats.Msg) error {
	...
	if err != nil {
		return fmt.Errorf("marshal universe failed event: %w", err)
	}
	...
	if _, err := handler.eventPublisher.PublishMsg(failedMessage, nats.Context(ctx)); err != nil {
		return fmt.Errorf("publish universe failed event: %w", err)
	}
	return nil
}
