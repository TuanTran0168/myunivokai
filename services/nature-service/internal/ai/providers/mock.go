...
// GenerateStructured simulates an AI model: it reads the same user prompt a
// real model would receive, picks a Nature DNA preset that fits the requested
// mood, and personalizes the landmarks from the user's interests/traits. See
...
func (p *MockProvider) GenerateStructured(ctx context.Context, req ai.StructuredRequest) (*ai.StructuredResponse, error) {
	...
	return &ai.StructuredResponse{Provider: ai.ProviderMock, Model: "mock-forest-dna-v1", JSON: payload, Usage: simulatedUsage}, nil
}

// mockProfile is the subset of the user profile the mock needs, extracted from
// the structured prompt built by prompts.ForestDNAUserPrompt.
