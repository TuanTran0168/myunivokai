...
// NewWorldSeed uses a NAT- prefix (universe-service uses WLD-) so a seed's
// owning service is readable at a glance in logs and shared URLs.
func NewWorldSeed() (string, error) {
	suffix, err := randomBase32(10)
	if err != nil {
		return "", err
	}
	return "NAT-" + suffix, nil
}
