...
func (s *PostgresStore) CreateWorld(ctx context.Context, world models.World, variant models.WorldVariant, logs []models.AIGenerationLog) (WorldBundle, error) {
	...
	dnaJSON, err := json.Marshal(world.NatureDNA)
	if err != nil {
		return WorldBundle{}, fmt.Errorf("marshal nature dna: %w", err)
	}
	configJSON, err := json.Marshal(variant.Config)
	if err != nil {
		return WorldBundle{}, fmt.Errorf("marshal scene config: %w", err)
	}
	row := tx.QueryRow(ctx, `INSERT INTO worlds (nickname, role, input, nature_dna, archetype, scene_name, quote, visibility)
...
const worldSelectColumns = `id::text, nickname, COALESCE(role,''), input, nature_dna, archetype, scene_name, quote,
...
func scanWorld(scanner rowScanner) (models.World, error) {
	...
	if err := json.Unmarshal(dnaJSON, &world.NatureDNA); err != nil {
		return models.World{}, fmt.Errorf("decode nature dna for %s: %w", world.ID, err)
	}
	world.ShortNarrative = world.NatureDNA.ShortNarrative
	return world, nil
}
