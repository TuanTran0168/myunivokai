...
func TestEveryMutationEmitsAWorldChangeEvent(t *testing.T) {
	...
	for _, mutation := range mutations {
		t.Run(mutation.name, func(t *testing.T) {
			...
			message := messages[0]
			if message.Subject != contracts.NatureWorldChangedEventSubject {
				t.Fatalf("%s published on %q, want %q", mutation.name, message.Subject, contracts.NatureWorldChangedEventSubject)
			}
			...
			if snapshot.Family != contracts.WorldFamilyNature || snapshot.WorldID != worldID {
...
func TestCreateWorldCarriesTheFirstSnapshotOnTheCompletedEvent(t *testing.T) {
	...
	if messages[0].Subject != contracts.NatureCompletedEventSubject {
		t.Fatalf("CreateWorld published on %q, want %q", messages[0].Subject, contracts.NatureCompletedEventSubject)
	}
