...
| [vision/service-wake-mechanism.md](vision/service-wake-mechanism.md) | Sleeping services never wake from a NATS message. Proactive wake on write, reactive on read, the three-way status-code contract, and the removal path once off free tier. **Implemented**; §What was built records where the code differs from the design | Before touching gateway error handling, `/healthz`, or the wake platform adapters |
