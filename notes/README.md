...
| `vision/auth-and-admin-plan.md` | Proposed, not implemented, with amendments | Drafted 2026-08-05 and amended the same day: one gateway with an admin route group, Django-style runtime-editable RBAC, and a token/role model that must extend to 3D-web accounts. Seven owner decisions at the end are unanswered |
